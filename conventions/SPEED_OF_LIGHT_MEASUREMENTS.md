...
| O2 delta, four files | 2-3% calibrated against B1 (5.2 MB; latency- and apply-bound) | 1.66-1.87 s NVMe / 2.75-2.85 s HDD evicted this build versus 1.71-1.91 s / 2.82-3.02 s shipped; same-state force-redownload upper bound is 2.167 s versus 45.315 s, 20.91x faster | 5.2 MB transferred, 97% affected-file byte savings; five paired patch/full-repository samples and ten oracle passes, Sept 18 `20260918T050136Z-16949de4` | Byte savings are established. The force-redownload arm transferred all 217 files, so an affected-files-only full-download control is still required before attributing the full elapsed difference to patching |
