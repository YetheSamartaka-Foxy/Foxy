...
| O7 database persistence | Per-kind calibrated estimate is now available for fully classified gate-1 records; existing accepted action runs used gate 4 and remain unrated | Force redownload: 604 write calls in 16 categories, 125-126 ms of gated windows inside a 7.5 s action (delta lane); WAL 125 ms vs MVCC 210 ms small, 11.6 s vs 37.4 s large (Sept 9) | Sept 16 evidence; per-kind instrumentation added Sept 18 | WAL remains the measured choice; gate-4 summed transaction windows are not critical-path wall time, so a clean gate-1 O7 reference run remains to be recorded |
