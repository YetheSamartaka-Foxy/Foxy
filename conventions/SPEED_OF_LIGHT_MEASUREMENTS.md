...
| O8 startup to settled verdict | No physical action ratio; observed dependency bound covers 100% of startup wall time with a zero recorded join | 575-680 ms total, first frame 433-484 ms, dispatch 509-614 ms and verdict 55-66 ms; the required completion is `max(first_frame, dispatch + verdict)`, never their sum | Clean commit `69e1033`, five samples, Sept 18 `20260918T150930Z-04c6b228` | The dependency accounting is complete and explicitly non-physical; the independently calibrated startup probe remains the network reference |
