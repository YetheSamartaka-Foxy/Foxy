...
6. **Counters and timers**: `SOL op=download` at the end of the transfer
   ...
   active files and ranges. `Download final wave:` records bytes remaining
   when one file remains active after the per-mod queue empties, plus time
   from that point to the last byte. This differs from the plateau-defined
   `tail_s`. `-- DOWNLOAD REPORT --`
