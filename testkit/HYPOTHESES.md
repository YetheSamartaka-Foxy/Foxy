...
| open | 128 MiB rotational reads gain again (the gain per doubling fell from 5% to 3%) | full recheck elapsed | lower, about 1-2% | `perf-hdd-plan-main-recheck-hdd-screen` |
| open | An unbuffered overlapped reader (`FILE_FLAG_NO_BUFFERING`, aligned buffers, two to four reads in flight) closes the SSD gap to the 6.1 GB/s 32-reader reference | full recheck elapsed on SSD | lower, possibly 15-18 s from 30 s | `perf-hdd-plan-main-recheck-ssd` |
| open | A trusted verified-hash record keyed by NTFS file ID, size, mtime and the USN journal lets a bootstrap after a database wipe skip files the journal proves untouched | bootstrap elapsed | lower by most of the payload; a product decision first, since it proves less than a re-read and never applies to an integrity recheck | `perf-hdd-plan-main-recheck-hdd` |
