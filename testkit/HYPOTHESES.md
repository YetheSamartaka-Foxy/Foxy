...
| accepted | Reading the Windows power plan costs about 0.1 s per call and the hash scheduler read it once per hash batch; reuse a sample up to 30 s old and take the first one off-thread at sync start | `sync_action` on the force-redownload tail | lower: per-mod hash batch 0.11 to 0.03 s, `perf-db-parts-bulk` 8.96 to 6.59 s median of 3, SSD recheck 15.58 to 15.38 s | `perf-db-parts-bulk`, `perf-hdd-plan-main-recheck-ssd` |
| closed | The streamed part groups run slower than the engine allows | streamed group insert elapsed | n/a, at the floor: 9.4-11 us a row in the app against 9-10 in `bench_streamed_groups_real_manifests` after a table drop; two groups cost the same as one flush (3.84 s against 3.78-3.83 s); inserting into the pages the purge freed adds about 0.37 s | `perf-hdd-plan-main-bootstrap-record-hdd` |
| rejected | Inserting real part rows in index key order (by path) instead of entry order speeds the insert | insert elapsed | lower: 3.20-3.22 s against 3.29-3.34 s, 3% | `bench_subfiles_real_manifest_order` |
| rejected | Committing the last streamed group in the background overlaps it with the tree load and hash restore | record restore elapsed | lower: 16.51 s instead of 7.1 s; the tree builds addons from `addon_files`, which that group still held, so the restore saw 6 of 96 addons and rehashed; waiting for the links leaves about 0.4 s to overlap | `perf-hdd-plan-main-bootstrap-record-hdd` |
...
| closed | Sort the 433k-part reload in process instead of with `ORDER BY` | `select subfiles` elapsed | n/a, already in place (`sort_parts_for_tree_load`); found stale on 2026-09-24 | `bench_bulk_read_path` |
| closed | Replace the correlated subqueries in `final_progress_flush` with an `UPDATE ... FROM` join | `update download_target_file` elapsed | n/a, already a join; its 3.2 s on `perf-db-parts-bulk` is the wait behind the part insert on the single writer | `perf-db-parts-bulk` |
| closed | Move the skip checks in `tree_hash_bootstrap` and `existing_graph_queue_rebuild` ahead of the work they discard | stage elapsed | n/a, both stages measure 0.0 s on the force-redownload now | `perf-db-parts-bulk` |
| closed | Run single-statement seam writes in autocommit instead of BEGIN/statement/COMMIT | seam statement count | n/a, `execute_retry` already runs in autocommit under WAL | `perf-db-parts-bulk-profiled` |
| closed | Skip the per-mod patch-table deletes when the repository has no patch rows | `delete download_patch_op` calls | n/a, patch clears are already buffered into one `flush_pending_patch_clears` | `perf-db-parts-bulk-profiled` |
